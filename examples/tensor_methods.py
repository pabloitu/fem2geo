...
def plot_dilation_tendency(sigma, n_strikes=180, n_dips=45):
    ...
    # added \\ instead of \ to resolve SyntaxWarning: invalid escape sequence '\s'
    fig.colorbar(cax, cax=cbaxes, shrink=0.4,
...
##added Dec 2025 by Cecile + Muriel
#summarised_tendency is the sum of slip_tendency and dilation_tendency.
def get_summarised_tendency(sigma, p_disc):
    """
    Gets the slip+dilation 'summarised' tendency a given stress tensor and plane.
    used by e.g. Jolie et al. (2016, Geology)
    ...
    if p_disc.ndim == 1:
        n = tr.plane_sphe2enu(p_disc)
        t = np.dot(sigma, n)
        #sigma_n as defined in get_slip_tendency:
        sigma_n = np.dot(t, n)
        #sigma_n as defined in get_dilation_tendency is: sn = np.dot(n_hat, np.dot(sigma, n_hat))
        sigma_s, sigma_s_dir = resshear_enu(p_disc, sigma)

        return abs(sigma_s / sigma_n) + ((s1 - sigma_n) / (s1 - s3))
    else:
        for plane in p_disc:
            n = tr.plane_sphe2enu(plane)
            t = np.dot(sigma, n)
            ...
            D.append(abs(sigma_s / sigma_n) + (s1 - sigma_n) / (s1 - s3))
...
##added Dec 2025 by Cecile + Muriel
#plots a third graph to the right: the summarised dendency Tsd=Ts+Td
def plot_slipndilationnsummarised_tendency(sigma, n_strikes=181, n_dips=46):
    """
    Plots jointly the slip and dilation and summarised tendency 
    for a tensor projected onto all planes from a
    polar discretization
    ...
    # Summarised Slip + Dilation tendency
    plane_data_ = np.array([([i[0], i[1]]) for i
                           in np.nditer(((mesh_strikes[:-1, :-1] + mesh_strikes[:-1, 1:]) / 2,
                                         (mesh_dips[:-1, :-1] + mesh_dips[1:, :-1]) / 2))])
    #should this call "plane_data" as in Slip or "norm" as in Dilation?
    D_sd = get_summarised_tendency(sigma, plane_data_)
    lon_sd, lat_sd = mpl.pole(mesh_strikes, mesh_dips)
    D_reshaped_sd = D_sd.reshape((mesh_strikes.shape[0] - 1, mesh_strikes.shape[1] - 1))


    fig = plt.figure(figsize=(27, 8))

    ax_s = fig.add_subplot(131, projection='stereonet')
    ax_s.grid()
    ax_d = fig.add_subplot(132, projection='stereonet')
    ax_d.grid()
    ax_sd = fig.add_subplot(133, projection='stereonet')
    ax_sd.grid()
    
    # changed cmap from 'rainbow' to 'jet' to have the same across 3 plots
    cax = ax_s.pcolormesh(lon_s, lat_s, D_reshaped_s, cmap='jet', shading='auto')
    
    #updated numbers so that 3 colourbars plot at the right place
    cbaxes_s = fig.add_axes([0.36, 0.1, 0.02, 0.3])  # Add additional axis for colorbar
    fig.colorbar(cax, cax=cbaxes_s, shrink=0.4)

    cax = ax_d.pcolormesh(lon_d, lat_d, D_reshaped_d, cmap='jet', shading='auto')

    cbaxes_d = fig.add_axes([0.625, 0.1, 0.02, 0.3])  # Add additional axis for colorbar
    fig.colorbar(cax, cax=cbaxes_d, shrink=0.4)
    
    cax = ax_sd.pcolormesh(lon_sd, lat_sd, D_reshaped_sd, cmap='jet', shading='auto')

    cbaxes_sd = fig.add_axes([0.92, 0.1, 0.02, 0.3])  # Add additional axis for colorbar
    fig.colorbar(cax, cax=cbaxes_sd, shrink=0.4)

    
    # return fig, ax_s, ax_d, D_reshaped_s, D_reshaped_d, (mesh_strikes, mesh_dips)
    return fig, ax_s, ax_d, ax_sd, D_reshaped_s, D_reshaped_d, D_reshaped_sd, (mesh_strikes, mesh_dips)
    
##added Dec 2025 by Cecile
#plots the summarised dendency Tsd=Ts+Td
def plot_summarised_tendency(sigma, n_strikes=181, n_dips=46):
    """
    Plots the summarised tendency 
    for a tensor projected onto all planes from a
    polar discretization
    ...
    ax_sd = fig.add_subplot(111, projection='stereonet')
    ax_sd.grid()
        
    cax = ax_sd.pcolormesh(lon_sd, lat_sd, D_reshaped_sd, cmap='jet', shading='auto')

    cbaxes_sd = fig.add_axes([0.9, 0.1, 0.02, 0.3])  # Add additional axis for colorbar
    fig.colorbar(cax, cax=cbaxes_sd, shrink=0.4)

    
    # return fig, ax_s, ax_d, D_reshaped_s, D_reshaped_d, (mesh_strikes, mesh_dips)
    return fig, ax_sd, D_reshaped_sd, (mesh_strikes, mesh_dips)